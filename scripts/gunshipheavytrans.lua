...
local collisionImmunityTime = 3000 --frames
...
local function ReapplyCollisionDamage(unitID)
	Sleep(collisionImmunityTime)
...
function ForceDropUnit()
	if (unitLoaded ~= nil) and Spring.ValidUnitID(unitLoaded) then
		...
		GG.SetCollisionDamageMult(unitLoaded, 0)
		StartThread(ReapplyCollisionDamage, unitLoaded)
	end
	unitLoaded = nil
...
function script.Killed(recentDamage, maxHealth)
	-- keep the unit technically alive (but hidden) for some time so that
	-- It can reapply collision to it's former passenger
	return GG.Script.DelayTrueDeath(unitID, unitDefID, recentDamage, maxHealth, Killed, collisionImmunityTime+30) --
end
